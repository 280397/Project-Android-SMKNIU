...
public class ScanAdminAddActivity extends AppCompatActivity implements ZXingScannerView.ResultHandler {
    ...
    String et_tgl_kembali, et_keperluan, id_user_pjmm, id_admin;
...
//        scannerView.resumeCameraPrevie
    ...
    private void checkadmin(final String admin) {
        Call<ResponseAdmin> checkadmin = Initretrofit.getInstance().getAdmin(admin);
        checkadmin.enqueue(new Callback<ResponseAdmin>() {
            @Override
...
//                            Log.d("tag",""+);
                            pinjam(Prefs.getString(SharedPreferences.getId(),""),et_tgl_kembali,data.getIdAdmin(),et_keperluan);
...
//                            Log.d(TAG, "onClick: "+data.getId()+"--"+SharedPreferences.getId()+"--"+data.getBarcode());
                    ...
                    dialog.setPositiveButton("ok", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialogInterface, int i) {
                            ...
                            Toast.makeText(ScanAdminAddActivity.this, "Barcode not registered!", Toast.LENGTH_SHORT).show();
                            finish();
                        }
                    });
                    show = dialog.create();
